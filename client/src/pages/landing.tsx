...
import { ScrollReveal, FadeIn, StaggerChildren, StaggerItem, NumberTicker } from "@/components/ui/motion-primitives";
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (statsRef.current) {
        const statElements = statsRef.current.querySelectorAll("[data-gsap-stat]");
  ...
  return (
    <MarketingLayout>
      <section className="relative overflow-hidden">
        <div ref={heroRef} className="relative mx-auto max-w-6xl px-4 py-20 md:py-32 text-center">
          <FadeIn delay={0.1}>
          ...
          <FadeIn delay={0.5}>
            <div className="mt-10 flex flex-wrap justify-center gap-4">
              <Button size="lg" className="h-12 px-8 text-base shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30 transition-shadow duration-300" asChild data-testid="button-hero-start-simulation">
                <Link href="/auth">
                  Start Simulation
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Link>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="text-center">
                    ...
                    <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500 rounded-full transition-all duration-1000 ease-out" style={{ width: "72%" }} />
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-primary rounded-full transition-all duration-1000 ease-out" style={{ width: "65%" }} />
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-violet-500 rounded-full transition-all duration-1000 ease-out" style={{ width: "84%" }} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <StaggerChildren className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {howItWorks.map((item) => (
              <StaggerItem key={item.step}>
                <div
                  className="relative rounded-xl glass-subtle p-6 hover:shadow-lg transition-shadow duration-300 h-full"
                  data-testid={`card-step-${item.step}`}
                >
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-primary-foreground text-sm font-bold shadow-lg shadow-primary/20">
                      {item.step}
                    </div>
          ...
          <StaggerChildren className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {coreFeatures.map((f) => (
              <StaggerItem key={f.title}>
                <div
                  className={`rounded-xl border p-8 bg-gradient-to-br ${f.gradient} hover:shadow-lg hover:-translate-y-1 transition-all duration-300`}
      ...
      <section className="border-t">
        <div className="mx-auto max-w-6xl px-4 py-20 md:py-24">
          ...
          <StaggerChildren className="grid grid-cols-1 gap-6 md:grid-cols-2 max-w-3xl mx-auto">
            {differentiators.map((d) => (
              <StaggerItem key={d.label}>
                <div
                  className={`rounded-xl border p-6 transition-all duration-300 hover:shadow-md ${d.negative ? "bg-muted/30 opacity-60" : "bg-gradient-to-br from-primary/5 to-violet-500/5 border-primary/20 hover:-translate-y-0.5"}`}
      ...
      <section className="border-t bg-card/20">
        <div ref={statsRef} className="mx-auto max-w-6xl px-4 py-16 md:py-20">
          <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center" data-testid={`stat-${stat.label.toLowerCase().replace(/\s+/g, "-")}`} data-gsap-stat>
