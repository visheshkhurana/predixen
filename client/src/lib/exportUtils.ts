...
export async function downloadSimulationPDF(data: SimulationPDFData) {
  ...
  doc.text(`${data.companyName || 'Predixen Intelligence OS'}  |  ${dateStr}  |  1,000 Monte Carlo Runs`, margin, 34);
  ...
  const addFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text('Generated by Predixen Intelligence OS  |  For authorized recipients only', margin, pageH - 9);
