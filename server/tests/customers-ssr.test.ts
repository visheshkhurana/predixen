// /customers was a blank Googlebot shell (ssr_chars=0). Pin a non-empty
// #ssr-content body plus the product / features / compare / pricing / free-tool / auth hrefs.
//
// Run: npx tsx --test server/tests/customers-ssr.test.ts
...
test("/customers SSR body is non-empty and links evaluation destinations", () => {
  ...
  assert.match(text, /have not published customer names/i);
  ...
  assert.equal(canon, "https://founderconsole.ai/customers");

  const page = jsonLdBlocks(out).find((b) => (b as { "@type"?: string })["@type"] === "WebPage") as
    | { url?: string }
    | undefined;
  assert.ok(page, "WebPage JSON-LD missing");
  assert.equal(page?.url, "https://founderconsole.ai/customers");
});
