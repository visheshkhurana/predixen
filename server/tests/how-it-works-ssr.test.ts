// /how-it-works was a blank Googlebot shell (ssr_chars=0). Pin a non-empty
// #ssr-content body plus the product / features / use-cases / free-tool / pricing / auth hrefs.
//
// Run: npx tsx --test server/tests/how-it-works-ssr.test.ts
...
test("/how-it-works SSR body is non-empty and links product destinations", () => {
  ...
  assert.match(text, /Truth Scan/i);
  assert.match(text, /Monte Carlo/i);
  assert.match(text, /P10\/P50\/P90/);
  assert.match(text, /not a timeline, a success rate, or a customer story/i);
  ...
  assert.equal(canon, "https://founderconsole.ai/how-it-works");

  const page = jsonLdBlocks(out).find((b) => (b as { "@type"?: string })["@type"] === "WebPage") as
    | { url?: string }
    | undefined;
  assert.ok(page, "WebPage JSON-LD missing");
  assert.equal(page?.url, "https://founderconsole.ai/how-it-works");
});
