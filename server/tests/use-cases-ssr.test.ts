// /use-cases was a blank Googlebot shell (ssr_chars=0). Pin a non-empty
// #ssr-content body plus the product / features / free-tool / industry / pricing / auth hrefs.
//
// Run: npx tsx --test server/tests/use-cases-ssr.test.ts
...
test("/use-cases SSR body is non-empty and links product destinations", () => {
  ...
  assert.match(text, /runway planning/i);
  assert.match(text, /have not published customer stories/i);
  ...
  assert.equal(canon, "https://founderconsole.ai/use-cases");

  const page = jsonLdBlocks(out).find((b) => (b as { "@type"?: string })["@type"] === "WebPage") as
    | { url?: string }
    | undefined;
  assert.ok(page, "WebPage JSON-LD missing");
  assert.equal(page?.url, "https://founderconsole.ai/use-cases");
});
