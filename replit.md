...
-   **Copilot Quick Chat with Web Research**: POST /api/companies/{id}/quick-chat endpoint for natural language financial queries. Context pack includes 12 months financial history, uploaded metric points, simulation summaries. Keyword-based detection triggers Perplexity web search for market benchmarks, industry trends, and competitor data. Responses combine internal company data with real-time web research. Frontend Cmd+K drawer with ReactMarkdown rendering, conversation history, source citations (Truth Scan, Financial Records, Uploaded Data, Simulation Results, Web Research, Connected Services), and follow-up suggestions.
-   **Strategic Web Search Module** (`server/copilot/web_search.py`): 8 specialized Perplexity search types (competitor_analysis, benchmark_lookup, market_sizing, cogs_research, market_entry, pricing_research, trend_analysis, general_research) with custom system prompts for each. Auto-triggers based on keyword/decision pattern detection in user queries.
-   **Business Context Engine** (`server/copilot/business_context.py`): Pulls comprehensive data from multiple sources: Truth Scan (primary), FinancialRecords (historical trends for revenue, burn, margin, CAC), uploaded metrics, simulations, and connector metadata (Shopify, Stripe, GA, Hubspot, Gusto, etc.). Identifies data gaps and suggests which connectors to link.
-   **Consultant-Grade Copilot Persona**: System prompts upgraded to McKinsey + a16z partner persona. Structured responses with sections: Financial Assessment, Market Research, Customer Fit, Unit Economics, Recommendation (GO/NO-GO/CONDITIONAL), Risks, Implementation Roadmap. Opinionated recommendations backed by data, using frameworks (TAM/SAM/SOM, Porter's Five Forces, unit economics).
-   **Web Research & Data Gaps in UI**: Copilot chat responses include `web_research_used`, `web_research_citations`, `web_research_type`, and `data_gaps` fields. Frontend displays purple "Web Research" badges on messages enriched with Perplexity data, and orange "Data Gap" badges suggesting which connectors to add. Conversation memory tracks web research metadata across exchanges.
