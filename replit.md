...
16. **Cap Table Management (Carta-like)**: Full equity management system at `/cap-table` with structured database tables (`shareholders`, `equity_holdings`, `option_grants`, `equity_transactions`, `valuations_409a`). Features: shareholder registry (founder/employee/investor/advisor types), equity issuance with share class (common/preferred/series), share transfers with balance checks, option grants with ISO/NSO/RSA/RSU types and 4y-1y-cliff vesting, option exercise converting to common, transaction audit log, 409A valuation tracking, dilution scenario modeling, and ownership pie chart. API: `/api/companies/{id}/cap-table/*`. Frontend: 6 tabs (Ownership, Shareholders, Option Grants, Transactions, Dilution Model, 409A). Demo data seeded for TechFlow Analytics (company 168).
...
-   **Decision engine calibration**: Escalating risk penalties for burn increases: >20% gets -0.10, >40% adds -0.25, >60% adds -0.25, >80% adds -0.15 (cumulative -0.75). Runway penalties: <12mo adds -0.12, <9mo adds -0.10 more. Risk text warns about burn increases.
-   **AI Decision Summary**: Burn-increase verdict override: GO downgrades to CONDITIONAL GO when burn increase ≥40% (lowered from 50%). Key Risk prioritizes burn warnings. Uses actual burn data, not name heuristics.
-   **Briefing progress indicator**: `LOADING_STEPS` in `decisions.tsx` total 73s (8+15+20+30). Fourth step: "Generating strategic briefing".
...
-   **StrategyCard burn warning**: Cards with `burnChange >= 40%` show a red warning with tiered advice (40-49%, 50-74%, 75%+). `data-testid="warning-high-burn-{strategyId}"`.
-   **KPI Board metric consistency**: KPI Board now uses `sharedMetrics` (useFinancialMetrics) as primary data source for burn and runway, matching Dashboard. Churn normalized to decimal before `formatPercent` (prevents 250% display bug). Runway capped at 60mo (matching Dashboard).
-   **Route redirects**: `/help` → `/docs`, `/data-input` → `/data`, `/simulate` → `/scenarios`. Prevents 404s on linked or typed URLs.
-   **Realtime API churn normalization**: Both snapshot and history endpoints normalize `churn_rate > 1` to decimal (divides by 100). Prevents double-multiplication in frontend `formatPercent`.
...
-   **Strategic briefing auto-generation**: GET `/strategic-diagnosis?auto_generate=true` triggers generation if no saved diagnosis exists. Frontend hook passes `auto_generate=true` so the briefing is generated on first page load using truth data. Sections: Executive Summary (situation_narrative), Recommendation (recommendation_headline + recommendation_narrative), Risks (key_risks), Execution Playbook (execution_playbook), Alternative Paths (alternative_paths). Falls back to data-driven template if LLM unavailable.
