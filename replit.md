...
-   **Key Features**:
    ...
    -   **Founder Autopilot**: Daily automated risk detection and briefing generation (`server/services/founder_autopilot.py`). Table: `autopilot_runs`. 5 risk detection rules (burn spike, low runway, MRR drop, churn spike, cash crisis). Generates natural language briefings with state snapshots. Also triggers monthly simulation accuracy computation.
    -   **Simulation Accuracy Tracker**: Compares past Monte Carlo predictions against actual financial records (`server/services/simulation_accuracy.py`). Tables: `simulation_accuracy`, `calibration_biases`. Computes per-metric accuracy scores (revenue, burn, cash) and systematic bias detection. Auto-calibration system (`server/services/calibration_bias.py`) analyzes prediction bias and applies corrections to future simulation inputs via `EnhancedSimulationEngine`. API endpoints at `/companies/{id}/simulation/accuracy`, `/simulation/accuracy/compute`, `/simulation/calibrate`, `/simulation/biases`. UI integrated into Simulate workspace History tab with Prediction Accuracy card, trend visualization, and auto-calibrate button.
    ...
    -   **Internal Admin Tools**: Admin System Tools page at `/admin/system` (`client/src/pages/admin/system-tools.tsx`). 5-tab UI: Events (event ledger viewer), Agents (AI governance dashboard), Flags (feature flag toggles), Autopilot (briefing viewer + run trigger), Intelligence (cross-company pattern stats + aggregation trigger).
    -   **Cross-Company Learning & Platform Intelligence**: Privacy-first cross-company benchmarking system. Companies opt in via `data_sharing_enabled` toggle (Company edit dialog). Only opted-in companies contribute anonymized data. Services: `server/services/data_anonymizer.py` (strips identifying info), `server/services/pattern_aggregator.py` (aggregates decision patterns + benchmarks, stores to `cross_company_patterns` table). Intelligence Graph, Decision Engine, Pattern Engine, and Copilot context enriched with cross-company insights. Admin endpoints: `GET /api/system/platform-intelligence` (stats), `POST /api/system/platform-intelligence/aggregate` (trigger). Only statistical aggregates (medians, percentiles, success rates) are exposed — never individual company data.
