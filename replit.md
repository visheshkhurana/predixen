...
-   **Frontend**: React 18, TypeScript, Wouter, Zustand, TanStack React Query, Tailwind CSS, shadcn/ui, Recharts for data visualization, React Hook Form with Zod for forms. Framer Motion for entrance/scroll/page animations. GSAP for advanced scroll-triggered effects. tailwindcss-motion plugin for CSS-based motion utilities.
    -   **Design System**: Glassmorphism tokens (`glass-subtle`, `glass-medium`, `glass-strong`, `glass-glow` CSS classes in `index.css`). Reusable motion primitives (`FadeIn`, `ScaleIn`, `StaggerChildren`, `StaggerItem`, `ScrollReveal`, `HoverLift`, `PressScale`, `PageTransition`, `NumberTicker`, `GlowEffect`) in `client/src/components/ui/motion-primitives.tsx`. Glass card components (`GlassCard`, `GlassPanel`, `GlassModal`) in `client/src/components/ui/glass-card.tsx`. AnimatePresence page transitions in both `App.tsx` (app routes via `AnimatedRouteWrapper`) and `MarketingLayout.tsx` (marketing routes). GSAP ScrollTrigger parallax on landing, features, pricing, and about pages (scoped via `gsap.context()` with `ctx.revert()` cleanup). All animations respect `prefers-reduced-motion`. Dark mode is primary theme.
...
-   **Key Features**:
    ...
    -   **Paywall & Subscription System**: 4-tier plan system (Free $0, Starter $29, Growth $49, Scale $99/month) with 30-day free trial. Plan definitions in `server/core/plans.py`. Subscription logic in `server/core/subscription.py` with `PaywallMiddleware` for server-side route gating and `check_feature_access()` dependency for per-endpoint gating. Frontend: `PaywallGate` component (`client/src/components/PaywallGate.tsx`) with `TrialBanner`, `useHasFeature` hook, and `useSubscription` hook (`client/src/hooks/use-subscription.ts`). Billing page at `/billing` (`client/src/pages/billing.tsx`). Premium routes wrapped with `GatedRoute` in `App.tsx`. Stripe not yet connected — subscription system is infrastructure-ready for Stripe integration. Trial abuse prevention: one trial per user enforced in `start_trial()`.
