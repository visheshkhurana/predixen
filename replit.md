...
-   **Key Features**:
    ...
    -   **Simulation Console** (`/simulate`): Live 3-column layout with real-time metric cards (left), cash timeline + agent event feed (center), investor outlook + cohort retention + decision replay (right). Uses agent-based simulation engine with 5 AI agents (Founder, Investor, Customer, Team, Market). CSS-only animations via `client/src/styles/simulation-animations.css`, animated counters via `client/src/hooks/useCounter.ts`.
    -   **Agent Simulation Engine** (`server/simulation_agents/`): Multi-agent orchestrator with knowledge graph, agent memory, environment engine. API at `/api/companies/{id}/simulation/agent-run`. Shareable results via `/api/simulation/shared/{token}`.
